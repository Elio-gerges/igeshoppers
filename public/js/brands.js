...
const User_UICONTROLLER = (function () {
    const DOMstrings = {
        ...
        informationHolder: '#cardBrandInfo',
        btnCloseInformationHolder : '#btnCloseBrandInformationHolder',

        frmDetails: "#frmBrandDetails",
        ...
        columnsConfiguration: [
            ...
            { 
                ...
                sortable: false,
                render: function(data, cell, row) {
                    cell.setAttribute("brand_id", data);
                    return `<i class="far fa-eye ${DOMstrings.animator.replace(".", "")}" brand_id="${data}"></i>`;
                }
            },
        ],
        ...
        tableLabels: {
            ...
            perPage: "{select} brands per page",
        ...
        exportOptions : {
            filename: "Shoppers Back Office - Brands List - " + COMMON_UICONTROLLER.getFormatDate(new Date()),
...
// brands APP CONTROLLER
var UIBrandsModalController = (function(UICtrl, APICtrl) {
    ...
    function onAddData(e) {
        try {
            ...
            swal({
                title: "Are you sure?",
                text: "You are about to create a new Brand!",
                icon: "warning",
                buttons: true,
                dangerMode: true,
              })
                ...
                if (willUpdate) {
                    ...
                    .then(result => {
                        if(result.error) {
                            ...
                            swal(`Here You Go! Brand added successfully!`, {
                                icon: "success",
                            });
                            hideInformation();
                            getAPIData();
                    ...
                    .catch(err => {
                        COMMON_UICONTROLLER.showErrorMessage("Error!", "Failed to Add Brand");
                        console.log('err @ Brands.js', err);
                    });
                } else {
                  swal({
                    title: "Okay!",
                    text: "Adding Brand Discarded!"
                });
                }
            });
        } catch (error) {
    ...
    function onUpdateData(e) {
        try {
            e.preventDefault();
            var newBrand = getFormData();

            var changedData = COMMON_UICONTROLLER.findChanges(newBrand, selectedData);
            if(Object.keys(changedData).length === 0) {
                swal({
                    title: "Brand Information are still the same!",
                    text: "You haven't changed the information of this Brand!\nOperation to Update is not allowed!",
                    icon: "warning",
                });
            } else {
                var message = `You are attempting to change the information of the Brand: ${selectedData.name}!`;
                swal({
                    title: "Are you sure?",
                    text: message,
                    icon: "warning",
                ...
                .then((willUpdate) => {
                    if (willUpdate) {
                        APICtrl.updateBrand(selectedID, changedData)
                        .then(response => response.json())
                        .then(result => {
                            if(result.error) {
                                COMMON_UICONTROLLER.showErrorMessage("Error!", result.error_message);
                                ...
                                swal(`Here You Go! Brand ${selectedData.name} updated successfully!`, {
                                    icon: "success",
                                });
                                hideInformation();
                                getAPIData();
                        ...
                        .catch(err => {
                            COMMON_UICONTROLLER.showErrorMessage("Error!", "Failed to Update Brand");
                        });
                    } else {
                        swal({
                            title: "Okay!",
    ...
    function showInformation(e) {
        ...
        if(e) {
            ...
            selectedID = e.target.getAttribute("brand_id");
            APICtrl.getBrand(selectedID)
            .then(response => response.json())
            .then(result => {
                populateFormData(result.brand);
                selectedData = result.brand;
    ...
    function populateFormData(brand) {
        getQS(DOM.txtName).value = brand.name;
        getQS(DOM.txtDescription).value = brand.description;
        var status = document.getElementsByName('status');
        status.forEach(s => {
            if(s.value === (brand.status === true ? "active" : "inactive" )) {
                s.selected = true;
            }
        });
    }
    ...
    function getAPIData() {
        ...
        .then(response => response.json())
        .then(result => {
            loadDataTable(consumeData(result.brandsData.list));
        })
        .catch(err => {
